...
insert_text(GtkWidget*widget, gpointer data) {
  ...
  gtk_entry_buffer_insert_text(GTK_ENTRY_BUFFER(input->buff),f+1, text, 1);
}
          ...
          gpointer        user_data)
{
  ...
  gtk_grid_attach (GTK_GRID (grid_numb), calc_data->sin_button, 1, 1, 1, 1);
  gtk_grid_attach (GTK_GRID (grid_numb), calc_data->cos_button, 2, 1, 1, 1);
  gtk_grid_attach (GTK_GRID (grid_numb), calc_data->tan_button, 3, 1, 1, 1);
