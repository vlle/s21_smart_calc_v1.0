...
void draw_function(GtkDrawingArea *area, cairo_t *cr, int width, int height,
  ...
  cairo_paint(cr);
  const char *t_x1 =
      gtk_entry_buffer_get_text(GTK_ENTRY_BUFFER(calc_data->dom_buff));
  const char *t_x2 =
      gtk_entry_buffer_get_text(GTK_ENTRY_BUFFER(calc_data->mdom_buff));
  const char *t_y1 =
      gtk_entry_buffer_get_text(GTK_ENTRY_BUFFER(calc_data->mcdom_buff));
  const char *t_y2 =
      gtk_entry_buffer_get_text(GTK_ENTRY_BUFFER(calc_data->cdom_buff));
  ...
  width += fabsl(min_domain) + fabsl(max_domain) * 100 / 3;
  height += fabsl(min_codomain) + fabsl(max_codomain) * 100 / 3;
...
void activate(GtkApplication *app, gpointer user_data) {
  ...
  gtk_entry_buffer_set_text(calc_data->buff, "sin(x)", 7);
  gtk_entry_buffer_set_text(calc_data->mdom_buff, "3", 1);
  gtk_entry_buffer_set_text(calc_data->dom_buff, "-3", 2);
  gtk_entry_buffer_set_text(calc_data->mcdom_buff, "5", 1);
  gtk_entry_buffer_set_text(calc_data->cdom_buff, "-5", 2);
  ...
  gtk_grid_attach_next_to(GTK_GRID(grid_numb), finance_button,
                          calc_data->clear_button, GTK_POS_LEFT, 1, 1);
  gtk_grid_attach_next_to(GTK_GRID(grid_numb), draw_button, finance_button,
                          GTK_POS_LEFT, 1, 1);
  gtk_grid_attach_next_to(GTK_GRID(grid_numb), calc_data->sin_button,
                          calc_data->sqrt_button, GTK_POS_LEFT, 1, 1);
  gtk_grid_attach_next_to(GTK_GRID(grid_numb), calc_data->cos_button,
                          calc_data->ln_button, GTK_POS_LEFT, 1, 1);
  gtk_grid_attach_next_to(GTK_GRID(grid_numb), calc_data->tan_button,
                          calc_data->log_button, GTK_POS_LEFT, 1, 1);

  g_signal_connect(x_button, "clicked", G_CALLBACK(insert_text), calc_data);
  ...
  g_signal_connect(calc_data->fl_button, "clicked", G_CALLBACK(insert_text),
                   calc_data);
  g_signal_connect(finance_button, "clicked", G_CALLBACK(cb_create), NULL);

  gtk_grid_attach(GTK_GRID(grid_numb), calc_data->button_q, 0, 5, 1, 1);
  gtk_grid_attach_next_to(GTK_GRID(grid_numb), x_button, calc_data->button_q,
                          GTK_POS_LEFT, 1, 1);
